...
`eth-rlp-lib` is a small Rust implementation of Ethereum's RLP format.

The library crate name is `rlp`, so code examples import it as:
...
The project includes small binaries under `src/bin` for manual inspection with `println!`.

Examples:

```powershell
cargo run --bin encode_str -- dog
cargo run --bin decode_str -- 83646f67
cargo run --bin encode_u8 -- 127
cargo run --bin encode_u64 -- 1024
cargo run --bin encode_bool -- true
cargo run --bin trace_encode_length_be
```
