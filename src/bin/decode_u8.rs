...
fn main() {
    let hex = std::env::args().nth(1).unwrap_or_else(|| "7f".to_string());
    ...
    match decoded {
        RlpItem::Bytes(bytes) if bytes.len() == 1 => {
            println!("as u8: {}", bytes[0]);
        }
        RlpItem::Bytes(bytes) if bytes.is_empty() => {
            println!("as u8: 0");
        }
        _ => {
            println!("as u8: not a canonical single-byte integer view");
        }
    }
}
