//! Canonical SSZ test vectors.
//!
//! Every example from the EPF wiki and ethereum.org SSZ pages is verified here.
//! These are the reference values — if these pass, the implementation is correct.
...
fn encode_u64_1025_matches_spec() {
    // spec: uint64(1025).encode_bytes().hex() == '0104000000000000'
    assert_eq!(hex::encode(encode_u64(1025)), "0104000000000000");
}

#[test]
fn encode_u64_37_little_endian() {
    // 37 as u64 little-endian = [37,0,0,0,0,0,0,0]
...
#[test]
fn encode_u16_1025() {
    // 1025 = 0x0401 → LE = [0x01, 0x04]
    assert_eq!(encode_u16(1025), vec![0x01, 0x04]);
}

// ============================================================
// BASIC TYPES — decoding
// ============================================================
...
#[test]
fn decode_u64_1025() {
    // spec: uint64.decode_bytes(bytes.fromhex('0104000000000000')) == 1025
...
fn encode_vector_u64_256_512_768() {
    // spec: Vector[uint64,3](256,512,768).encode_bytes().hex()
...
fn encode_list_u64_1024_2048_3072() {
    // spec: List[uint64,5](1024,2048,3072).encode_bytes().hex()
...
fn encode_list_same_as_vector_for_fixed_elements() {
    // spec confirms: List[uint64,5] and Vector[uint64,3] produce same hex
    // when containing same elements
    let elements = vec![
        encode_u64(1024),
        encode_u64(2048),
        encode_u64(3072),
...
fn encode_bitvector_8_pattern() {
    // spec: Bitvector[8](0,0,1,0,1,1,0,1).encode_bytes().hex() == 'b4'
...
fn encode_bitvector_single_high_bit() {
    // spec: Bitvector[8](0,0,0,0,0,0,0,1).encode_bytes().hex() == '80'
...
fn encode_bitvector_5_compact() {
    // spec: Bitvector[5](1,0,1,0,1).encode_bytes().hex() == '15'
    // 1+(0<<1)+(1<<2)+(0<<3)+(1<<4) = 1+4+16 = 21 = 0x15
...
fn encode_bitlist_three_zeros() {
    // spec: Bitlist[100](0,0,0).encode_bytes().hex() == '08'
...
fn encode_bitlist_eight_zeros_needs_extra_byte() {
    // spec: Bitlist[8](0,0,0,0,0,0,0,0).encode_bytes().hex() == '0001'
...
fn encode_container_dummy_example() {
    ...
    assert_eq!(encoded, vec![
        37u8,0,0,0,
        55,0,0,0,
        16,0,0,0,  // offset = 16 (fixed part is 4+4+4+4=16 bytes)
        22,0,0,0,
        1,2,3,4,
    ]);
}
...
fn encode_container_alice_variable() {
    // spec: Alice(x=[1,2,3]).encode_bytes().hex() == '04000000010203'
    let encoded = encode_container(&[
...
fn encode_container_bob_fixed() {
    // spec: Bob(x=[1,2,3]).encode_bytes().hex() == '010203'
    let encoded = encode_container(&[
...
fn roundtrip_container_with_two_variable_fields() {
    // Container with two variable-size fields
    let encoded = encode_container(&[
        ContainerField::Variable(vec![1,2,3]),
        ContainerField::Variable(vec![4,5,6,7,8]),
    ]);
    // fixed_part_size = 4+4 = 8
    // offset0 = 8, offset1 = 8+3 = 11
